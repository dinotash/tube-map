...
station.prototype.writeLabel = function(dictionary) {
	if (!this.virtual) {
		...
		this.label.attr({"font-family": "johnston itc, gill sans", "text-anchor": anchor, "font-size": 7, "fill": "black"});
...
station.prototype.changeColour = function(overrideColour, beenDone) {
	...
	if (!this.virtual) {
		...
		if ((typeof this.joinedStation != "undefined") && (!beenDone)) {
			if (overrideColour == "undefined") { //reset
		...
		for (var i = 0; i < this.symbols.length; i++) {
			...
			var type = thisSymbol.data("type");
			if (type == "tick") {
				if ((overrideColour == "undefined") || (overrideColour == undefined)) { //reset
					var defaultFill = thisSymbol.data("defaultFill");
					thisSymbol.attr({fill: defaultFill});
				}
				else {
